...
const { generate: idGenerate, validate: idValidate } = create(
  'wertyuiopasdfghjklzxcvbnm1234567890QWERTYUIOPASDFGHJKLÑZXCVBNM;:_,.-\'ç*+`^"}{][?¡!"·$%&/()=ªº\\'
)(36);
const gPrefix = "sl";
...
export const validateToken = async (
  ...
  const dbResults = await db
    .select()
    .from(apiTokens)
    .where(eq(apiTokens.apiToken, token))
    .execute();
